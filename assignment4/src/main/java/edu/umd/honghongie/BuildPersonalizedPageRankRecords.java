...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static final String SOURCE_NODES = "source_nodes";
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    static ArrayList<String> snodes = new ArrayList<String>();  //save id of source nodes
    private static ArrayListOfFloatsWritable pageranklist = new ArrayListOfFloatsWritable(); 


    //Initialize node type and pagerank value for all nodes 
    @Override
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
      ...
      String tt = context.getConfiguration().get(SOURCE_NODES, null);
      if (tt!=null){
        String ss[] = tt.split(",");
      ...
      if (tt==null){
        throw new RuntimeException(SOURCE_NODES + "is null!");
      }
    ...
    //build adjacent neighbor of nodes 
    @Override
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      //if arr[0].equals(SOURCE_NODE); then change setPagerand to 1
      //System.out.println("SOURCE_NODE: "+SOURCE_NODE+ "***:its value is: "+tt);
      //System.out.println("arr[0]: "+ arr[0]);
      for (int i = 0; i < snodes.size(); i++){
        if (arr[0].equals(snodes.get(i))){
          pageranklist.set(i, 0);
        }else{
          pageranklist.set(i, Float.NEGATIVE_INFINITY);
        }
      }
      node.setPageRank(pageranklist);
      ...
      if (arr.length == 1) {
        ...
        for (int i = 1; i < arr.length; i++) {
          neighbors[i - 1] = Integer.parseInt(arr[i]); //rest nodes to adjacent structure
        }
      ...
      context.write(nid, node); //nodeid and adjacent nodes
    }
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    if (!cmdline.hasOption(INPUT) || !cmdline.hasOption(OUTPUT) 
        || !cmdline.hasOption(NUM_NODES) || !cmdline.hasOption(SOURCE)) {
